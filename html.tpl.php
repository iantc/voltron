...
<head profile="<?php print $grddl_profile; ?>">
  ...
  <script type="text/javascript">
    jQuery(document).ready(function() {
	 ...
	 jQuery('#block-jquerymenu-1 li.parent span.parent').next('a').each(function(){
       var p = jQuery(this);
	   p.html(
         p.text().replace(/\(\d\)/,'')
       )
	   jQuery(this).removeAttr('href').addClass('parent');
     });
	 jQuery('#block-jquerymenu-1 li.parent span.parent.closed').append(' <i class="icon-chevron-right"></i>');
	 jQuery('#block-jquerymenu-1 li.parent span.parent.open').append(' <i class="icon-chevron-down"></i>');
	 jQuery("a.parent").click(function() {
       jQuery(this).prev("span.parent").children("i").toggleClass("icon-chevron-right").toggleClass("icon-chevron-down");
     });
